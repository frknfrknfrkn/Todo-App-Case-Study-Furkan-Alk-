  ...
  <div className="overflow-x-auto scroll-x-container">
    <div className="flex gap-3 pb-2">
      {categories.map((cat) => (
        <div
          key={cat.id}
          className="bg-white border rounded-xl p-3 shadow-sm flex flex-col justify-between items-center text-sm min-w-[160px] max-w-[180px] h-[130px]"
        >
          {editCategory?.id === cat.id ? (
            ...
            <>
              <span
                className="inline-block px-2 py-1 rounded text-white text-xs text-center w-full truncate"
  ...
  {Array.isArray(todos) && todos.map((todo) => (
    <div key={todo.id} className="bg-white border rounded-xl shadow-sm p-4 sm:table-row sm:p-0 sm:border-0 sm:shadow-none text-sm">
      <div className="sm:table-cell sm:p-2">
        <span className="block sm:hidden font-medium text-gray-500">Başlık</span>
        {todo.title}
      </div>
      <div className="sm:table-cell sm:p-2">
        <span className="block sm:hidden font-medium text-gray-500">Açıklama</span>
        <span className="text-gray-600 whitespace-pre-line">{todo.description || <em>—</em>}</span>
      </div>
      <div className="sm:table-cell sm:p-2 capitalize">
        <span className="block sm:hidden font-medium text-gray-500">Durum</span>
        {todo.status}
      </div>
      <div className="sm:table-cell sm:p-2">
        <span className="block sm:hidden font-medium text-gray-500">Kategoriler</span>
      ...
      <div className="sm:table-cell sm:p-2 capitalize">
        <span className="block sm:hidden font-medium text-gray-500">Öncelik</span>
        {todo.priority === "low" && "Düşük"}
        {todo.priority === "medium" && "Orta"}
        {todo.priority === "high" && "Yüksek"}
      </div>
      <div className="sm:table-cell sm:p-2">
        <span className="block sm:hidden font-medium text-gray-500">Aksiyonlar</span>
